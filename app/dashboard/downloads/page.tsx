...
export default function DownloadsPage() {
  ...
  return (
    <div className="container p-0 mx-auto space-y-6 md:space-y-8">
      <div className="text-center max-w-3xl mx-auto mb-4 md:mb-8 px-4">
        <h1 className="text-xl md:text-3xl font-bold tracking-tight bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">
          Download CREST VPN
        </h1>
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-100 overflow-hidden mx-4 md:mx-0">
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="p-4 md:p-8 flex flex-col justify-center">
            <div className="w-fit mb-2 px-2 py-1 bg-emerald-50 text-emerald-700 text-xs rounded-full font-medium">RECOMMENDED</div>
            <h2 className="text-xl md:text-2xl font-bold mb-2">CrestVPN for Windows</h2>
            ...
            <div className="flex flex-wrap gap-2 md:gap-4 mb-4 md:mb-6">
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-emerald-50 flex items-center justify-center">
                  <Check className="h-3 w-3 md:h-3.5 md:w-3.5 text-emerald-600" />
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs md:text-sm">Latest v8.2.1</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-emerald-50 flex items-center justify-center">
                  <Check className="h-3 w-3 md:h-3.5 md:w-3.5 text-emerald-600" />
                </div>
                <span className="text-xs md:text-sm">18.4 MB</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 md:gap-3">
              <Button
                ...
                className={`group bg-gradient-to-r ${isFreePlan() ? "from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700" : "from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700"} rounded-full text-white ${buttonAnimation} text-xs md:text-sm h-9 md:h-10`}
              >
                {isFreePlan() ? (
                  <Lock className={`mr-2 h-3.5 w-3.5 md:h-4 md:w-4 ${lockAnimation}`} />
                ) : (
                  <Unlock className={`mr-2 h-3.5 w-3.5 md:h-4 md:w-4 ${unlockAnimation}`} />
                )}
                {isFreePlan() ? "Premium Feature" : "Download Now"}
              </Button>
              <Button variant="outline" className="rounded-full text-xs md:text-sm h-9 md:h-10">
                <Info className="mr-2 h-3.5 w-3.5 md:h-4 md:w-4" />
                Release Notes
              </Button>
            </div>
          </div>
          <div className="relative bg-gradient-to-br from-emerald-50 to-teal-50 flex items-center justify-center p-4 md:p-8">
      ...
      {/* All Platforms */}
      <h2 className="text-lg md:text-xl font-semibold mt-8 md:mt-12 mb-4 md:mb-6 flex items-center px-4 md:px-0">
        <Shield className="mr-2 h-4 w-4 md:h-5 md:w-5 text-emerald-600" />
      ...
      <div className="mt-8 md:mt-12 bg-white rounded-xl border border-gray-100 p-4 md:p-6 shadow-sm mx-4 md:mx-0">
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="md:w-1/4">
            <div className="relative w-full max-w-[160px] md:max-w-[200px] mx-auto">
              <div className="absolute -inset-1 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-lg blur-lg"></div>
              <div className="relative bg-white rounded-lg p-4 shadow-sm">
                <Image
          ...
          <div className="md:w-3/4 text-center md:text-left">
            ...
            <div>
              <Button
                ...
                variant="outline"
                className={`group border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 ${buttonAnimation} text-xs md:text-sm h-9 md:h-10`}
              >
                {isFreePlan() ? (
                  <Lock className={`mr-2 h-3.5 w-3.5 md:h-4 md:w-4 ${lockAnimation}`} />
                ) : (
      ...
      {/* Help Section */}
      <div className="mt-8 md:mt-12 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-4 md:p-6 mx-4 md:mx-0">
    ...
    <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow transition-all duration-200">
      <div className="p-4 md:p-5">
        ...
        <div className="mt-4 space-y-2">
          <Button
            onClick={onDownload}
            className={`w-full text-xs md:text-sm h-8 md:h-9 ${isLocked() ? "bg-gray-100 text-gray-500 hover:bg-gray-200" : "bg-emerald-100 text-emerald-700 hover:bg-emerald-200"} rounded-lg`}
          >
            {isLocked() ? (
          ...
          {storeLink && (
            <Button
              variant="outline"
              className="w-full text-xs md:text-sm h-8 md:h-9 rounded-lg border-gray-200"
              asChild
            >
        ...
        {(setupGuides && setupGuides.length > 0) && (
          <div className="mt-4 pt-3 border-t border-gray-100">
            <p className="text-xs text-gray-500 mb-2">Setup Guides:</p>
            <ul className="space-y-1">
              {setupGuides.map((guide, index) => (
                <li key={index}>
                  <Link 
                    href={guide.url}
                    className="text-xs md:text-sm text-emerald-600 hover:text-emerald-700 flex items-center"
                  >
                    <div className="w-1 h-1 rounded-full bg-emerald-400 mr-2"></div>
                    {guide.title}
                  </Link>
                </li>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              {variants.map((variant, index) => (
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="h-7 text-xs border border-gray-200 rounded-md"
