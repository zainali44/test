...
export default function SubscriptionsPage() {
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-6">
          ...
          {addOns.map((addon, index) => (
            <div key={addon.name}>
              <div className="flex items-center justify-between p-3">
                <div className="flex items-center">
                  <div className="w-7 h-7 rounded-sm bg-gray-50 flex items-center justify-center mr-3">
                    <addon.icon className="h-3.5 w-3.5 text-gray-700" />
                  </div>
                  <div>
                    <div className="flex items-center">
                      <span className="text-xs font-medium">{addon.name}</span>
                      {addon.isNew && (
                        <Badge className="ml-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white border-0 text-[9px] px-1.5 py-0 rounded-sm">
                          NEW
                        </Badge>
                      )}
                    </div>
                    {addon.status === "locked" ? (
                      ...
                      <div className="flex items-center text-emerald-600 text-[10px] mt-0.5">
                ...
                {addon.status === "locked" ? (
                  <Button
                    size="sm"
                    className="bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 text-white text-[10px] h-7 px-2.5 rounded-sm"
                  >
                    Get Add-on
                  </Button>
                ) : (
                  <Button
                    variant="outline"
                    size="sm"
                    className="text-gray-900 border-gray-300 hover:bg-gray-50 text-[10px] h-7 px-2.5 rounded-sm"
                  >
                    Configure
                  </Button>
                )}
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-medium text-gray-900">VPN Credentials</h2>
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="text-[10px] h-6 px-1 text-gray-500 hover:bg-transparent rounded-sm"
                >
                  <Info className="h-3.5 w-3.5" />
                </Button>
              </TooltipTrigger>
              <TooltipContent side="left" className="text-[10px] p-2 max-w-[200px] bg-white">
                <p>Use these credentials when you want to connect VPN manually.</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
          ...
          <div className="p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-gray-500">VPN username</div>
                  <div className="text-xs font-medium mt-1">{subscriptionData.username}</div>
                </div>
              </div>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className="text-gray-900 hover:bg-gray-100 text-[10px] h-6 px-2 rounded-sm"
                    >
                      {usernameCopied ? (
                  ...
                  <TooltipContent className="text-[10px] p-2 bg-white">
                    <p>{usernameCopied ? "Copied!" : "Copy username"}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          ...
          <div className="p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-gray-500">VPN password</div>
